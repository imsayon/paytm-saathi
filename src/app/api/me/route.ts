...
import { supabaseConfigured } from "@/server/auth/supabase";
  ...
  return handle(request, async () => {
    ...
    try {
      ...
      return NextResponse.json({
        ...
        auth_configured: supabaseConfigured(),
        demo_mode: config.demoMode,
        merchant: { id: ctx.merchantId, name: ctx.merchantName, demo_session: ctx.isDemoSession },
        user: ctx.user ? { email: ctx.user.email, phone: ctx.user.phone } : null,
      });
    } catch (error) {
      if (error instanceof AppError && error.code === "UNAUTHENTICATED") {
        return NextResponse.json({ signed_in: false, auth_configured: supabaseConfigured(), demo_mode: config.demoMode, merchant: null, user: null });
      }
      throw error;
    }
  });
